    ...
    if (found) {
      ...
      items.push({
        ...
        why: e.source === 'catalog'
          ...
          : `You quoted this ${e.times} time${e.times === 1 ? '' : 's'}, last on ${e.last_on || 'a past job'} at $${round2(e.price)}.`,
        matched_from: clause,
      });
    } else {
      items.push({
  ...
  return items.map(it => {
    ...
    if (found && found.score >= 0.5) {
      ...
      out.why = e.source === 'catalog'
        ...
        : `Your own price: quoted ${e.times}×, last on ${e.last_on || 'a past job'}.`;
...
module.exports = { draft, localDraft, ask, polish, ready, config, buildMatcher, parseQty, clauses, reground };
