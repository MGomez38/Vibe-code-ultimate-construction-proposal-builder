...
module.exports = function makeFinance(db) {
  /** What the bench actually logged against a ticket or job. */
  function usageCost(where, id) {
    if (!id) return 0;
    return db.prepare(`SELECT COALESCE(SUM(qty * unit_cost), 0) s FROM material_usage WHERE ${where} = ?`).get(id).s;
  }

  function woFinancials(wo) {
    const items = parseItems(wo.items);
    // Planned items are the estimate; logged usage is what the shop really pulled.
    // Once anything is logged it supersedes the plan, so a ticket is never costed twice.
    const logged = usageCost('work_order_id', wo.id);
    const materialCost = logged > 0 ? logged : itemsCost(items);
    ...
    return {
      ...
      logged_material_cost: round2(logged), planned_material_cost: round2(itemsCost(items)),
    };
  }
  ...
  function jobFinancials(jobId) {
    ...
    const materialCost = db.prepare('SELECT * FROM job_materials WHERE job_id = ?').all(jobId)
      ...
      + usageCost('job_id', jobId);   // what the crew logged straight against the job
