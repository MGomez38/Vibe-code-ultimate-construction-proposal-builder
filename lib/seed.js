...
module.exports = function seed(db) {
  ...
  // ---------------------------------------------------------------- inventory
  const insMat = db.prepare(`INSERT INTO materials (company_id, sku, name, category, unit, qty_on_hand, reorder_point, unit_cost, sell_price, location, vendor) VALUES (?,?,?,?,?,?,?,?,?,?,?)`);
  ...
  const asMat = {};
  asStock.forEach(m => { asMat[m[0]] = insMat.run(AS, ...m).lastInsertRowid; });
  ...
  const dtsMat = {};
  dtsStock.forEach(m => { dtsMat[m[0]] = insMat.run(DTS, ...m).lastInsertRowid; });
