...
It was built against a real shop price book — a 34-sheet fabrication workbook — and handles what that turned up: a sheet with no header row at all (set the header row yourself), section headings like *Galvanized and Bond* / *Stainless Steel* / *5052 Aluminum* that become the category and keep `26ga 48" x 120"` under two sections from colliding into one item, a separate grade column (GALV, SS304) that joins the description, and a **last-priced date**. Rows priced at $0 are refused rather than imported as free line items.

That date then does real work: material older than two years is flagged in red on the inventory list, and any quote drawing on it says so — *"Last priced 2011-12-07 — 14 years ago. Check it before you send this."*

**Fitting tables import too.** A fabrication book does not list saddles the way it lists sheet metal — it has one row per size with the finished price beside it, and often a second axis across the top (girth down the side, length across, a price at every crossing). Switch the importer to **A size table**, say what the thing is called and which column holds the size and the price, and every size becomes a quotable item: *Roof saddle 24"*, *Flex round 10" round*, *Register tap 20" girth × 12"*. The second axis is found by pointing at the row that labels it. Where a sheet stacks several tables down the page it says so — the same size appearing twice at two prices is a warning, not a silent duplicate — and you set the last row and import each table under its own name.

**Duct is priced by formula, not by grid.** A fabrication price grid is a photograph of one moment — it holds whatever gauge, liner and labor class the workbook was set to when it was saved. Import the grid and you inherit that snapshot; 24ga with half-inch liner and nothing else quotable. So the importer has a third mode that reads the **formula behind the grid** instead:
...
**Your price book ships with the app.** The All Spec estimating workbook has been decoded into data the app loads on first run — 49 materials, the duct formula with all 170 timed sizes and 5 gauges, and 369 priced ell sizes, the transition calculator, and eight fitting tables — Ductmate 35 at list and at the 0.8 discount, round and square flex, plain and caulked round caps, and chimney saddles and uppers. There is no import step and no spreadsheet in the loop.

**Quoting is a picker.** *Quote Duct & Fittings* in the sidebar: tap what you are quoting, pick the size, say how many, hit **Add to quote**. Lines stack up with a running total; name it, pick the customer, and **Create the quote** makes a real quote you can email. Duct and transitions are computed so any gauge, liner and opening works; ells and the rest come straight off your own tables. The picker is grouped by what the work actually is — **Ductwork** and **Chimney & roof** — so chimney saddles are not sitting in the middle of the duct list for an estimator to scroll past.
